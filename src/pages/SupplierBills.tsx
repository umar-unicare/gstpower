...
export default function SupplierBills() {
  ...
  const getFileUrl = (file: SupplierBillFile): string => {
    // If file has url (from backend), use it directly
    if (file.url) return file.url;
    
    // Otherwise, create blob URL from dataUrl (for upload preview)
    if (file.dataUrl) {
    ...
    if (file.type === 'application/pdf') {
      ...
      printWindow.document.write(`
        ...
        <html>
          ...
          <body>
            <img src="${fileUrl}" alt="${file.name}" onload="window.print()" />
          </body>
        </html>
      `);
      printWindow.document.close();
