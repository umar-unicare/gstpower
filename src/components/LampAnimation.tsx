...
interface LampAnimationProps {
  onLampToggle?: (isOn: boolean) => void;
}

const LampAnimation = ({ onLampToggle }: LampAnimationProps) => {
  ...
  useEffect(() => {
    ...
    const CORDS = gsap.utils.toArray<SVGPathElement>(".cords path");
    const HIT = containerRef.current.querySelector(".lamp__hit") as SVGCircleElement;
    const DUMMY_CORD = containerRef.current.querySelector(".cord--dummy") as SVGLineElement;
    ...
    const CORD_TL = gsap.timeline({
      ...
      onStart: () => {
        ...
        gsap.set(DUMMY_CORD, { opacity: 0 });
        gsap.set(CORDS[0], { display: "block" });

        onLampToggle?.(STATE.ON);
      },
      onComplete: () => {
        gsap.set(DUMMY_CORD, { opacity: 1 });
  ...
  return (
    <div ref={containerRef} className="lamp-container">
      <svg
        ref={lampRef}
        className="lamp"
        viewBox="0 0 333 484"
